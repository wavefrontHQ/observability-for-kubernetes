  ...
  stages {
    stage("Set RUN_CI") {
      steps {
        script {
          env.RUN_CI = sh(
            script: 'if [[ "$(git rev-parse --abbrev-ref HEAD)" == "main" ]]; then git diff --quiet --name-only --diff-filter=ADMR ${GIT_COMMIT}~..${GIT_COMMIT} -- operator scripts collector && echo false || echo true; else git diff --quiet --name-only --diff-filter=ADMR origin/main..${GIT_COMMIT} -- operator scripts collector && echo false || echo true; fi',
            returnStdout: true
          ).trim()
        }
      }
