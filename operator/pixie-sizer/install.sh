...
sed "s/PS_TRAFFIC_SCALE_FACTOR_VALUE/$PS_TRAFFIC_SCALE_FACTOR/g" "$PIXIE_SIZER_YAML" | \
  sed "s/PS_MEASURE_MINUTES_VALUE/$PS_MEASURE_MINUTES/g" | \
  kubectl apply -f -

# Give useful commands
echo
echo "Wait $PS_MEASURE_MINUTES minutes, then run the following command to see the recommendation:"
echo "    kubectl --namespace observability-system logs --selector=\"app.kubernetes.io/component=pixie-sizer\" --container=pixie-sizer --since=\"${PS_MEASURE_MINUTES}m\" | grep 'recommended settings'"
