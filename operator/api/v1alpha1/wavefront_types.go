...
type Hub struct {
	...
	// Cloud Account ID that the collector is running in. In AWS, this will be the AWS Account ID.
	ClusterCloudAccountID string `json:"clusterCloudAccountID,required"`

	// Cloud provider the Kubernetes cluster is hosted in. Valid values include `AWS,?????`.
	ClusterCloudProvider string `json:"clusterCloudProvider,required"`

	// Whether the cluster is self-managed (`false`), or managed by a cloud provider (`true`).
	ManagedCluster bool `json:"managedCluster,omitempty"`

	// Region that the cluster is running in. For on-prem clusters, the value here should be `on-prem,???`.
	ClusterRegion string `json:"clusterRegion,required"`

	// CSP client ID that was generated by Secure State for this collector.
	CSPClientID string `json:"cspClientID,required"`

	// CSP Organization ID that this collector is part of.
	CSPOrgID string `json:"cspOrgID,required"`

	// URL of the CSP instance to connect to.
	// +kubebuilder:default:="https://console.cloud.vmware.com"
	CSPUrl string `json:"cspUrl,required"`

	// TODO: Ask Tim about valid values
	// Whether the environment is demo, or stage, or prod.
	// +kubebuilder:default:=prod
	Environment string `json:"environment,required"`

	// +kubebuilder:default:=0
	// +kubebuilder:validation:Enum:=0;1
	TelegrafTraceEntityID int `json:"telegrafTraceEntityID,required"`

	// +kubebuilder:default:=0
	// +kubebuilder:validation:Enum:=0;1
	TelegrafTraceMetric int `json:"telegrafTraceMetric,required"`

	// LeMans TSS metrics domain
	// +kubebuilder:default:=aria-k8s-prod-us-2
	TSSMetricsDomain string `json:"tssMetricsDomain,required"`

	// LeMans TSS metrics storage policy
	// +kubebuilder:default:=aria-k8s
	TSSMetricsStoragePolicy string `json:"tssMetricsStoragePolicy,required"`

	// LeMans TSS metrics endpoint (prometheus remote write protocol)
	// +kubebuilder:default:="https://data.mgmt.cloud.vmware.com/le-mans/v1/streams/tss-proto"
	TSSMetricsUrl string `json:"tssMetricsUrl,required"`

	// +kubebuilder:default:="30s"
	VSSNetworkMapMetricsCollectIntervalSeconds string `json:"vssNetworkMapMetricsCollectIntervalSeconds,required"`

	// +kubebuilder:default:="http://aria-telegraf-collector.aria-k8s:9273/metrics"
	VSSNetworkMapMetricsUrl string `json:"vssNetworkMapMetricsUrl,required"`

	// ID of the collector that was generated by Secure State.
	VSSCollectorID string `json:"vssCollectorID,required"`
